...
\begin{subbox}{}
    \begin{itemize}
        ...
        \item Die $k$-te Ableitung an der Stelle $t = 0$ ist das $k$-te Moment von $X$: $M_X^{(k)}(0) = m_k := \E(X^k).$ 
        \item Das $k$-te zentrale Moment von $X$ (mit $\mu = \E(X)$): $\mu_k := \E\left((X-\mu)^k\right).$ 
