...
\begin{enumerate}
    ...
    $\limn \P(|X_n - Z| > \varepsilon) \centernot\longrightarrow 0$ für $n \to \infty$, gilt  
...
\begin{mainbox}{schwaches Gesetz der grossen Zahlen}
    Sei $X_1, X_2, ...$ eine Folge von paarweise unkorrelierten Zufallsvariablen, die alle den gleichen Erwartungswert $\E(X_i) = \mu$ und die gleiche Varianz $\Var(X_i) = \sigma^2$ haben.
    Sei 
    $$\overline{X}_n = \frac{1}{n}S_n = \frac{1}{n}\sum_{i = 1}^{n}X_i$$
...
beziehungsweise
$$S_n \approx \mathcal{N}(n\mu, n \sigma^2) \text{ und } \overline{X}_n \approx \mathcal{N}\left(\mu, \frac{1}{n}\sigma^2\right) \quad \text{(approximativ für grosse } n)$$
