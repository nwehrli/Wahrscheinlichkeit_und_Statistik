...
\begin{enumerate}
    ...
    \item Wir haben eine Äquivalenz: 
    \begin{align*}
...
\begin{subbox}{linearer Transformationssatz}
    ...
    $$H(\omega) = g(Z(\omega))$$
    ein $m$-dimensionaler Zufallsvektor und ferner gilt
    $$\P(H \in A) = \P(Z \in g^{-1}(A)).$$
    Wenn $g$ linear und umkehrbar (i.e. $g(x) = m + Bx$ mit det$(B) \neq 0$) und unter Voraussetzung, dass die Verteilung von $Z$ absolut stetig ist, dann ist $H$ auch absolut stetig und es gilt:
...
\begin{align*}
    ...
    &\text{Substitution } v = xy, \mathop{dv} = y\mathop{dx}\\
...
\end{align*}
$$\implies f_V(v) = \frac{\ln(v)}{v^2}\mathds{1}_{v \in [1,\infty)}$$
