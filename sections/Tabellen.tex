...
\begin{center}
	\begin{tabularx}{\linewidth}{XX}
		...
		$\limxi (1+\frac{1}{x})^b = 1$                         & $\limn n^{\frac{1}{n}} = 1$             \\
...
\begin{itemize}
	...
	\item Sei $\mathcal{X}, Y$ kompakt, $f: Y \subset \R^n \to \R$ stetig. 
	
	Sei $\gamma: \mathcal{X} \to Y$ mit $\mathcal{X} = \mathcal{X}_0 \cup B, Y = Y_0 \cup C$ ($B, C$ Rand von $\mathcal{X}, Y$). 
	
	Wenn $\gamma: \mathcal{X}_0 \to Y_0$ bijektiv und $C^1$ mit det$(J_\gamma(x)) \neq 0, \forall x \in \mathcal{X}_0$, dann gilt 
	\[\int_Y f(y)\mathop{dy} = \int_{\mathcal{X}} f(\gamma(x))|\text{det}(J_\gamma(x))|\dx\]
\end{itemize}
...
\begin{itemize}
	...
	\item $X_1, ..., X_n \sim \mathcal{N}(\theta_1, \theta_2)$ iid. : $T_{\theta_1} = \overline{X}_n, \ T_{\theta_2} = \frac{1}{n}\sum_{i=1}^n (X_i - \overline{X}_n)^2$
\end{itemize}
