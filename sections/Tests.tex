...
\[\forall \theta \in \Theta_0 \quad \P_\theta(T \in K) \le \alpha\]
...
\[\beta : \Theta_A \to [0,1], \quad \theta \mapsto \P_\theta(T \in K)\]
...
Wenn \(R \gg 1\), sprechen die Daten eher für \(H_A\) als für \(H_0\); analog spricht \(R \ll 1\) eher für \(H_0\).
...
\begin{enumerate}
	...
	\item Test \(T = \frac{\overline{X}_n - \theta_0}{\sqrt{\sigma^2/n}} \sim \mathcal{N}(0,1)\) unter \(\P_{\theta_0}\)
...
\begin{enumerate}
	...
	\item Verwerfungsbereich: \(K_{>}\), \(K_{<}\) oder \(K_{\neq}\)
...
\begin{enumerate}
	...
	\item \textbf{$\sigma^2$ unbekannt.} Empirische Varianzen der beiden einzelnen Datensätze
...
\begin{mainbox}{Definition p-Wert}
	...
	\[G: \Omega \to [0,1], \quad G(\omega) = \P_{\theta_0}[T \in K_{t(X_1(\omega), ..., X_n(\omega))}]\]
\end{mainbox}
